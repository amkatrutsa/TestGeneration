function beta = lars (X, y, method, stop, max_iters)%, useGram, Gram, trace)
...
%if strcmpi(method, 'lasso')	lasso = 1;
%else	lasso = 0;	end
[n p] = size(X);
X = X - ones(n,1)*mean(X);
...
% Assume stop is always positive
% We only need beta "vector"
beta = zeros(1, p);
beta_new = zeros(1, p);
...
% 	j = I(j);
	...
	GA1 = R\(R'\s);
       % if (any(isnan(GA1)) || any(isinf(GA1)))
       %     R = R+1e-10*speye(size(R));
       %     GA1 = R\(R'\s);
       %     disp('*')
       % end
	...
	if vars == nvars % if all variables active, go all the way to the lsq solution
		...
		a = Xt*u; % correlation between each variable and eqiangular vector
	...
	%%  I IS USED
        ...
        %size(temp)
        gamma = min([min(temp1(temp1> 0)), min(temp2(temp2> 0)), C/AA]);
        %gamma = min(min(temp(temp > 0)), C/AA);
	end
	% LASSO modification
	%if lasso
		lassocond = 0;
		temp = -beta(A)./w';
        %% XXX: Can be improved here
		[gamma_tilde] = min([temp(temp > 0) gamma]);
        %[gamma_tilde] = min(min(temp(temp > 0)), gamma);
    ...
    % XXX: we can just not save beta array..
